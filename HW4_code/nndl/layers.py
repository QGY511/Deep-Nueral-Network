...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    minibatch_normalized = (x - minibatch_mean) / np.sqrt(minibatch_var + eps)
    ...
    minibatch_normalized = (x - running_mean) / np.sqrt(running_var + eps)
    out = gamma * minibatch_normalized + beta
...
def batchnorm_backward(dout, cache):
  ...
  dgamma = np.sum(dout * minibatch_normalized, axis=0)


    
  numerator = x - minibatch_mean
