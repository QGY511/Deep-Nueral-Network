...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    softmax = lambda z: np.exp(z - np.max(z, axis=1, keepdims=True)) / np.sum(np.exp(z - np.max(z, axis=1, keepdims=True)), 
    ...
    # As done in HW2, for backward pass we simply minus 1 
    dscores = probs 
    dscores[range(N), y] =  dscores[range(N), y] -  1
    dscores =  dscores/N


    grads['W2'] = (a1.T @ dscores).T + reg * W2
    grads['b2'] = np.sum(dscores, axis=0)

    # Backprop into the second layer

    dh2= dscores @ W2
    dh2[a1 <= 0] = 0  # derivative for ReLU

    grads['W1'] = (X.T @ dh2).T + reg * W1
  ...
  def predict(self, X):
    ...
    relu = lambda z: np.maximum(0, z)
    a1 = relu(z1)
    scores = a1 @ self.params['W2'].T + self.params['b2'] 
