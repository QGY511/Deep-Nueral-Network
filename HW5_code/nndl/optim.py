...
def sgd_nesterov_momentum(w, dw, config=None):
  ...
  v = momentum * v_old - learning_rate * dw
...
def rmsprop(w, dw, config=None):
  ...
  next_w = w - (learning_rate)/(np.sqrt(a)+ epsilon) * dw
...
def adam(w, dw, config=None):
  ...
  bias_a = 1 - beta_2 ** t
  a = beta_2 * a + (1- beta_2) * dw * dw
  a_wave = a/bias_a

  next_w = w - (learning_rate)/(np.sqrt(a_wave) + epsilon ) * v_wave
